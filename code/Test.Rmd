...
Para revisar si las variables estan normalizadas se aplica el test de Shapiro Wilk en cada variables numérica de las antes mencionadas:
...
Así, para los datos como los generos que son boleanos, se aplica el test de Wilcoxon:
Probamos con drama:
```{r ,eval=TRUE,echo=TRUE}
wilcox.test(cleanData$metascore~cleanData$drama)
```
Probamos con comedy:
```{r ,eval=TRUE,echo=TRUE}
wilcox.test(cleanData$metascore~cleanData$comedy)
```
Por último, probamos con crime:
```{r ,eval=TRUE,echo=TRUE}
wilcox.test(cleanData$metascore~cleanData$crime)
```
...
```{r ,eval=TRUE,echo=TRUE}
cor.test(cleanData$drama,cleanData$must_see,method="spearman")
...
Vamos a probar con una regresión lineal simple, comparando la variable factor must_see con el numero de criticos:
...
modelo<-lm(formula = must_see ~ number_critics, data=train)
summary(modelo)
```

# Modelo no supervisado

Usaremos clusters, para ello primero aplicaremos el codo de Jambú par aescoger el número de clústeres que minimice la distancia intra-cluster:
```{r ,eval=TRUE,echo=TRUE}
...
El K=4 o 5, a partir de ahí los cambios no son significativos. 
...
cleanData.conf = fix.conf.table(cleanData.conf)
cleanData.conf
```
